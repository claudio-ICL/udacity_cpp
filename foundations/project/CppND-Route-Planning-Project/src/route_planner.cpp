...
RouteModel::Node *RoutePlanner::NextNode() {
  if (this->open_list.size()==0)
    return nullptr;
  sort(this->open_list.begin(), this->open_list.end(), CompareNodes);
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    assert (this->start_node!=nullptr);
    std::vector<RouteModel::Node> path_found{*current_node};
    distance = 0.0f;
    while ((current_node!=this->start_node)&(current_node->parent!=nullptr)){
...
void RoutePlanner::AStarSearch() {
    assert (this->start_node!=nullptr);
    assert (this->end_node!=nullptr);
    this->start_node->visited=false;
    this->end_node->visited=false;
    RouteModel::Node *current_node = this->start_node;
    this->open_list.clear();
    this->AddToOpen(current_node);
    while ((current_node!=this->end_node)&(this->open_list.size()>0)){
      this->AddNeighbors(current_node);
      current_node=this->NextNode();
    }
    if (current_node==this->end_node){
      std::cout << "A* terminates successfully\n";
      this->m_Model.path=this->ConstructFinalPath(current_node);
    }
    else{
      std::cout << "A* cannot find a solution\n";
    }
}
