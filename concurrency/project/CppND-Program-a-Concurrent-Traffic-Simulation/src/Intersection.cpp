...
{
    std::unique_lock<std::mutex> lck(_mtx);// the mutex _mtx is a private member of the base class TrafficObject and it is used to guard std::cout
    ...
    lck.lock();
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry. ";
    std::cout <<"TrafficLight is "<< ((_trafficLight.getCurrentPhase()==TrafficLightPhase::red)? "red":"green");
    std::cout << ".  ";
    std::cout<<std::endl;
    lck.unlock();
