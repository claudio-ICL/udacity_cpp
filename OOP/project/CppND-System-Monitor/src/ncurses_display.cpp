...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
  ...
  mvwprintw(window, ++row, 2,
            ("Total Processes: " + to_string(system.TotalProcesses()) /*+ "(" +
             to_string(system.Processes().size()) + ")"*/)
                .c_str());
  mvwprintw(
      window, ++row, 2,
...
void NCursesDisplay::DisplayProcesses(std::map<int, Process>& processes,
                                      std::vector<int> pids, WINDOW* window,
                                      int n) {
  int row{0};
  ...
  for (int i = 0; i < n; i++) {
    ...
    mvwprintw(window, row, user_column,
              (process.User() + "    ").substr(0, 9).c_str());
    float cpu = process.CpuUtilization() * 100;
    mvwprintw(window, row, cpu_column, to_string(cpu).substr(0, 4).c_str());
    mvwprintw(window, row, ram_column, process.Ram().substr(0, 6).c_str());
    mvwprintw(window, row, time_column,
              Format::ElapsedTime(process.UpTime()).c_str());
    mvwprintw(window, row, command_column,
              process.Command().substr(0, 40).c_str());
  }
}
...
void NCursesDisplay::Display(System& system, int n) {
  ...
  while (1) {
    ...
    DisplayProcesses(system.Processes(), system.Pids(), process_window, n);
    wrefresh(system_window);
    wrefresh(process_window);
    refresh();
